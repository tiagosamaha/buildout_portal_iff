...
<xsl:stylesheet version="1.0"
                ...
                exclude-result-prefixes="office meta config text table draw presentation
                                         dr3d chart form script style number anim dc xlink math xforms
		...
		<img>
			...
			<xsl:attribute name="src">
				<xsl:choose>
					...
					<xsl:when test="contains(@xlink:href, 'ObjectReplacements/')">
						<xsl:value-of select="substring-after(@xlink:href, 'ObjectReplacements/')" />
					</xsl:when>
					<xsl:otherwise>
